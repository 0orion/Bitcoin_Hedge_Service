...
def hedge():
    ...
    try:
        ...
        eclair_api = Eclair_API(
            base_url="http://localhost:8080", api_key="Your_Eclair_API_key"
        )
        channel_data = eclair_api.get_channel_info()

        if channel_data is None:
            logging.error("Failed to fetch channel data from Eclair API")
...
def _deposit_satoshis(amount):
    result = lnmarkets.deposit_satoshis(amount)
    return result

def _withdraw_satoshis(amount):
...
def deposit():
    ...
    try:
        ...
        result = _deposit_satoshis(amount)
        ...
        logging.exception("Error occurred while processing deposit")
        return jsonify({"error": "Internal Server Error"}), 500
...
def withdraw():
    ...
    try:
        ...
        result = _withdraw_satoshis(amount)
